...
## plot all presence and variation layers
```{r plot_p_u, warning = F}

plot(preds, col = viridis(50), bty="n", main = '', legend = F, axes = F, box = F)
plot(var, col = viridis(50), bty="n", main = '', legend = F, axes = F, box = F)
...
First step, create a DECIDE score for each species. So far, we have been doing this in four ways:
* $Variation$$
...
for(r in 1:4){
  ...
  hist(mn, main = paste0(names(dec_fls)[r], ', unweighted mean'), col=viridis(50), xlim = c(0,0.14))
  hist(mn_wt_var, main = paste0(names(dec_fls)[r], ', weighted by var'), col=viridis(50), xlim = c(0,0.14))
  hist(mn_wt_prob, main = paste0(names(dec_fls)[r], ', weighted by prob'), col=viridis(50), xlim = c(0,0.14))
  hist(q90, main = paste0(names(dec_fls)[r], ', 0.9 quantile'), col=viridis(50), xlim = c(0,0.14))
